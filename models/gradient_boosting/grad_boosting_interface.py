...
class Grad_Boosting_Interface:
    ...
    def train(self, train_X, train_y, val_X, val_y):
        ...
        self.gradBoosters = [Grad_Booster(learning_rate=0.01, boosting_rounds=5, max_depth=3, leaves=8),
                             Grad_Booster(
                                 learning_rate=0.9, boosting_rounds=1, max_depth=3, leaves=4),
        ...
        for i in range(3):
            ...
            train_zero_count, train_one_count, new_train_y = self.convert_data(
                train_y, pos_class, neg_class1, neg_class2)
            val_zero_count, val_one_count, new_val_y = self.convert_data(
                val_y, pos_class, neg_class1, neg_class2)
    ...
    def predict(self, test_X):
        ...
        for i in range(3):
            ...
            pred = [(math.exp(output)/(1 + math.exp(output)))
                    for output in pred]
            predictions[:, i] = pred
        return predictions
